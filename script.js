...
const arLowercase = ["abcdefghijklmnopqrstuvwxyz"];
const arUppercase = ["ABCDEFGHIJKLMNOPQRSTUVWXYZ"];
const arNumber = ["0123456789"];
const arSpecial = ["!@#$%^&*-_`~"];
    ...
    // shuffles main character list
    masterList.sort(() => Math.random() - 0.5)

    // sets length of passcode
    var postList = masterList.slice(0, promptLength);
    // combines array into single string
    var finalPass = (postList.join(''));
    // sends final passcode out
    return (finalPass);
}